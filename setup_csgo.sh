...
# 	su - steam -c "screen -S csgoServerInstall  ../setup_csgo.sh"
# 	echo "exit go go go ";
# 	exit
# 	cd ..
# fi 
...
DIR_CSGOSERVER="$DIR_SH/csgoserver"
DIR_STEAMCMD="${DIR_SH}/steamcmd"
echo "Create directory [ 创建目录“${DIR_CSGOSERVER}”和“${DIR_STEAMCMD}” ]"
chmod -R 777 *
mkdir $DIR_CSGOSERVER
mkdir $DIR_STEAMCMD
#进入steamcmd
cd $DIR_STEAMCMD


echo "Check < ${DIR_CSGOSERVER} > [ 检查Steamcmd... ]"
sleep 1s
path=steamcmd.sh #Steam.sh Found
if [ ! -f $path ];then
	...
	sleep 5s
	screen -L -S csgoServerInstall  ${DIR_SH}/setup_csgo.sh
	exit
	cd ..
fi
...
STEAM_RUNTIME=0 STEAMEXE=steamcmd ./steamcmd.sh +login anonymous +force_install_dir ${DIR_CSGOSERVER} +app_update 740 validate +quit
sleep 1s

 
echo "Check the CSGO Server files [ 检查CSGO服务器文件..... ] "
sleep 1s
path="$DIR_CSGOSERVER/srcds_run" #Server Found
if [ ! -f $path ];then
...
# cd /home
# sleep 5s
# screen -S csgoServerInstall  ./setup_csgo.sh
# exit
# cd ..
# fi
...
#cd ..
sleep 5s
screen -S csgoserver  ${DIR_SH}/runcsgo.sh
exit
cd ..
